    ...
    <body>
        ...
        <div class="col-12 col-sm-12 col-md-12 col-lg-6">
            <main class="container-fluid content-wrapper p-2 m-2">
                ...
                @csrf
                <h2>Создать бронь</h2>
                    <a class="h3 mb-3 font-weight-normal p-2"href="{{route('index')}}" role="button">Все брони</a>
                <div class="form-group">
                <label for="formGroupExampleInput">Пользователь</label>
                <select id="inputState" id="id_user" name="id_user" class="form-control">
                    @foreach($user as $el)
                    <option value = "{{$el->id}}">{{$el->fio}}</option>
                    @endforeach
                </select>
                    <small class="form-text text-muted">Выбери пользователь.</small>
                </div>
                <div class="form-group ">
                <label>Дата и время создания записи</label>
                <input type="datetime" class="form-control" id="date_time_szapisi" name="date_time_szapisi" value="{{ date('Y-m-d H:i:s')}}" readonly>
                <small class="form-text text-muted">Создается автоматически.</small>
                </div>
                <div class="form-group ">
                ...
                <div class="form-group ">
                <label for="inputState">Статус брони:</label>
                <select id="inputState" class="form-control" id="statys_broni" name="statys_broni">
